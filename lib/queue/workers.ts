...
import { sendWelcomeEmail, sendNewsSummaryEmail, clearTransporterCache } from '@/lib/nodemailer';
...
export const newsEmailWorker = new Worker(
    ...
    async (job) => {
        ...
        try {
            ...
            for (const { user, articles } of perUser) {
                try {
                    ...
                    if (errorMessage.includes('invalid_grant') || errorMessage.includes('Token has been expired')) {
                        console.error('❌ OAuth token expired. Clearing transporter cache...');
            ...
            await connection.del(lockKey);
            console.error('❌ News email job failed with error:', error);
            throw error;
        }
    },
    { connection }
